const ObjectId = require('pow-mongodb-fixtures').createObjectId;
exports.videos = {
  1: {
	"_id" : ObjectId("569190ca24de1e0ce2dfcd4f"),
	"titulo" : "Once Upon a Time in the West",
	"genero" : [
		"Western"
	],
	"anioPublicacion" : 1968,
	"duracion" : 175,
	"reparto" : [
		"Claudia Cardinale",
		"Henry Fonda",
		"Jason Robards",
		"Charles Bronson"
	],
	"director" : "Sergio Leone",
	"nominaciones" : {
		"wins" : 4,
		"nominations" : 5,
		"text" : "4 wins & 5 nominations."
  },
    costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
  },
2: {
	"_id" : ObjectId("569190cb24de1e0ce2dfcd50"),
	"titulo" : "A Million Ways to Die in the West",
	"genero" : [
		"Comedy",
		"Western"
	],
	"anioPublicacion" : 2014,
	"duracion" : 116,
	"reparto" : [
		"Seth MacFarlane",
		"Charlize Theron",
		"Amanda Seyfried",
		"Liam Neeson"
	],
	"director" : "Seth MacFarlane",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 6,
		"text" : "6 nominations."
  },
    costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
3: {
	"_id" : ObjectId("569190cb24de1e0ce2dfcd51"),
	"titulo" : "Wild Wild West",
	"genero" : [
		"Action",
		"Western",
		"Comedy"
	],
	"anioPublicacion" : 1999,
	"duracion" : 106,
	"reparto" : [
		"Will Smith",
		"Kevin Kline",
		"Kenneth Branagh",
		"Salma Hayek"
	],
	"director" : "Barry Sonnenfeld",
	"nominaciones" : {
		"wins" : 10,
		"nominations" : 11,
		"text" : "10 wins & 11 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
4: {
	"_id" : ObjectId("569190cb24de1e0ce2dfcd52"),
	"titulo" : "West Side Story",
	"genero" : [
		"Crime",
		"Drama",
		"Musical"
	],
	"anioPublicacion" : 1961,
	"duracion" : 152,
	"reparto" : [
		"Natalie Wood",
		"Richard Beymer",
		"Russ Tamblyn",
		"Rita Moreno"
	],
	"director" : "Jerome Robbins, Robert Wise",
	"nominaciones" : {
		"wins" : 18,
		"nominations" : 11,
		"text" : "Won 10 Oscars. Another 18 wins & 11 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
5: {
	"_id" : ObjectId("569190cb24de1e0ce2dfcd53"),
	"titulo" : "Slow West",
	"genero" : [
		"Action",
		"Thriller",
		"Western"
	],
	"anioPublicacion" : 2015,
	"duracion" : 84,
	"reparto" : [
		"Kodi Smit-McPhee",
		"Michael Fassbender",
		"Ben Mendelsohn",
		"Aorere Paki"
	],
	"director" : "John Maclean",
	"nominaciones" : {
		"wins" : 2,
		"nominations" : 9,
		"text" : "2 wins & 9 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
4: {
	"_id" : ObjectId("569190cb24de1e0ce2dfcd54"),
	"titulo" : "An American Tail: Fievel Goes West",
	"genero" : [
		"Animation",
		"Adventure",
		"Family"
	],
	"anioPublicacion" : 1991,
	"duracion" : 75,
	"reparto" : [
		"Phillip Glasser",
		"James Stewart",
		"Erica Yohn",
		"Cathy Cavadini"
	],
	"director" : "Phil Nibbelink, Simon Wells",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 0,
		"text" : ""
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
5: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd55"),
	"titulo" : "Red Rock West",
	"genero" : [
		"Crime",
		"Drama",
		"Thriller"
	],
	"anioPublicacion" : 1993,
	"duracion" : 98,
	"reparto" : [
		"Nicolas Cage",
		"Dennis Hopper",
		"Lara Flynn Boyle",
		"J.T. Walsh"
	],
	"director" : "John Dahl",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 3,
		"text" : "3 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
6: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd56"),
	"titulo" : "How the West Was Won",
	"genero" : [
		"Western"
	],
	"anioPublicacion" : 1962,
	"duracion" : 164,
	"reparto" : [
		"Carroll Baker",
		"Lee J. Cobb",
		"Henry Fonda",
		"Carolyn Jones"
	],
	"director" : "John Ford, Henry Hathaway, George Marshall, Richard Thorpe",
	"nominaciones" : {
		"wins" : 7,
		"nominations" : 5,
		"text" : "Won 3 Oscars. Another 7 wins & 5 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
7: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd57"),
	"titulo" : "Journey to the West",
	"genero" : [
		"Adventure",
		"Comedy",
		"Fantasy"
	],
	"anioPublicacion" : 2013,
	"duracion" : 110,
	"reparto" : [
		"Qi Shu",
		"Zhang Wen",
		"Bo Huang",
		"Show Luo"
	],
	"director" : "Stephen Chow, Chi-kin Kwok",
	"nominaciones" : {
		"wins" : 1,
		"nominations" : 12,
		"text" : "1 win & 12 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
8: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd58"),
	"titulo" : "West of Memphis",
	"genero" : [
		"Documentary"
	],
	"anioPublicacion" : 2012,
	"duracion" : 147,
	"reparto" : [
		"Michael Baden",
		"Jason Baldwin",
		"Holly Ballard",
		"Jamie Clark Ballard"
	],
	"director" : "Amy Berg",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 9,
		"text" : "Nominated for 1 BAFTA Film Award. Another 9 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
9: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd59"),
	"titulo" : "Star Wars: Episode IV - A New Hope",
	"genero" : [
		"Action",
		"Adventure",
		"Fantasy"
	],
	"anioPublicacion" : 1977,
	"duracion" : 121,
	"reparto" : [
		"Mark Hamill",
		"Harrison Ford",
		"Carrie Fisher",
		"Peter Cushing"
	],
	"director" : "George Lucas",
	"nominaciones" : {
		"wins" : 38,
		"nominations" : 27,
		"text" : "Won 6 Oscars. Another 38 wins & 27 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
10: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd5a"),
	"titulo" : "Star Wars: Episode V - The Empire Strikes Back",
	"genero" : [
		"Action",
		"Adventure",
		"Fantasy"
	],
	"anioPublicacion" : 1980,
	"duracion" : 124,
	"reparto" : [
		"Mark Hamill",
		"Harrison Ford",
		"Carrie Fisher",
		"Billy Dee Williams"
	],
	"director" : "Irvin Kershner",
	"nominaciones" : {
		"wins" : 15,
		"nominations" : 17,
		"text" : "Won 1 Oscar. Another 15 wins & 17 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
11:{
	"_id" : ObjectId("569190cc24de1e0ce2dfcd5b"),
	"titulo" : "Star Wars: Episode VI - Return of the Jedi",
	"genero" : [
		"Action",
		"Adventure",
		"Fantasy"
	],
	"anioPublicacion" : 1983,
	"duracion" : 131,
	"reparto" : [
		"Mark Hamill",
		"Harrison Ford",
		"Carrie Fisher",
		"Billy Dee Williams"
	],
	"director" : "Richard Marquand",
	"nominaciones" : {
		"wins" : 15,
		"nominations" : 15,
		"text" : "Nominated for 4 Oscars. Another 15 wins & 15 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
12: {
	"_id" : ObjectId("569190cc24de1e0ce2dfcd5c"),
	"titulo" : "Star Wars: Episode I - The Phantom Menace",
	"genero" : [
		"Action",
		"Adventure",
		"Fantasy"
	],
	"anioPublicacion" : 1999,
	"duracion" : 136,
	"reparto" : [
		"Liam Neeson",
		"Ewan McGregor",
		"Natalie Portman",
		"Jake Lloyd"
	],
	"director" : "George Lucas",
	"nominaciones" : {
		"wins" : 17,
		"nominations" : 59,
		"text" : "Nominated for 3 Oscars. Another 17 wins & 59 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
13: {
	"_id" : ObjectId("569190cd24de1e0ce2dfcd5d"),
	"titulo" : "Star Wars: Episode III - Revenge of the Sith",
	"genero" : [
		"Action",
		"Adventure",
		"Fantasy"
	],
	"anioPublicacion" : 2005,
	"duracion" : 140,
	"reparto" : [
		"Ewan McGregor",
		"Natalie Portman",
		"Hayden Christensen",
		"Ian McDiarmid"
	],
	"director" : "George Lucas",
	"nominaciones" : {
		"wins" : 21,
		"nominations" : 49,
		"text" : "Nominated for 1 Oscar. Another 21 wins & 49 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
14: {
	"_id" : ObjectId("569190cd24de1e0ce2dfcd5e"),
	"titulo" : "Star Trek",
	"genero" : [
		"Action",
		"Adventure",
		"Sci-Fi"
	],
	"anioPublicacion" : 2009,
	"duracion" : 127,
	"reparto" : [
		"Chris Pine",
		"Zachary Quinto",
		"Leonard Nimoy",
		"Eric Bana"
	],
	"director" : "J.J. Abrams",
	"nominaciones" : {
		"wins" : 22,
		"nominations" : 77,
		"text" : "Won 1 Oscar. Another 22 wins & 77 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
15: {
	"_id" : ObjectId("569190cd24de1e0ce2dfcd5f"),
	"titulo" : "Star Wars: Episode II - Attack of the Clones",
	"genero" : [
		"Action",
		"Adventure",
		"Fantasy"
	],
	"anioPublicacion" : 2002,
	"duracion" : 142,
	"reparto" : [
		"Ewan McGregor",
		"Natalie Portman",
		"Hayden Christensen",
		"Christopher Lee"
	],
	"director" : "George Lucas",
	"nominaciones" : {
		"wins" : 13,
		"nominations" : 47,
		"text" : "Nominated for 1 Oscar. Another 13 wins & 47 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
16: {
	"_id" : ObjectId("569190cd24de1e0ce2dfcd60"),
	"titulo" : "Star Trek Into Darkness",
	"genero" : [
		"Action",
		"Adventure",
		"Sci-Fi"
	],
	"anioPublicacion" : 2013,
	"duracion" : 132,
	"reparto" : [
		"Chris Pine",
		"Zachary Quinto",
		"Zoe Saldana",
		"Karl Urban"
	],
	"director" : "J.J. Abrams",
	"nominaciones" : {
		"wins" : 6,
		"nominations" : 44,
		"text" : "Nominated for 1 Oscar. Another 6 wins & 44 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
17: {
	"_id" : ObjectId("569190cd24de1e0ce2dfcd61"),
	"titulo" : "Star Trek: First Contact",
	"genero" : [
		"Action",
		"Adventure",
		"Sci-Fi"
	],
	"anioPublicacion" : 1996,
	"duracion" : 111,
	"reparto" : [
		"Patrick Stewart",
		"Jonathan Frakes",
		"Brent Spiner",
		"LeVar Burton"
	],
	"director" : "Jonathan Frakes",
	"nominaciones" : {
		"wins" : 8,
		"nominations" : 19,
		"text" : "Nominated for 1 Oscar. Another 8 wins & 19 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
17: {
	"_id" : ObjectId("569190cd24de1e0ce2dfcd62"),
	"titulo" : "Star Trek II: The Wrath of Khan",
	"genero" : [
		"Action",
		"Adventure",
		"Drama"
	],
	"anioPublicacion" : 1982,
	"duracion" : 113,
	"reparto" : [
		"William Shatner",
		"Leonard Nimoy",
		"DeForest Kelley",
		"James Doohan"
	],
	"director" : "Nicholas Meyer",
	"nominaciones" : {
		"wins" : 2,
		"nominations" : 9,
		"text" : "2 wins & 9 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
18: {
        "_id" : ObjectId("569190ce24de1e0ce2dfcd6b"),
        "titulo" : "From Russia with Love",
        "genero" : [
                "Action",
                "Adventure",
                "Thriller"
        ],
        "anioPublicacion" : 1963,
        "duracion" : 115,
        "reparto" : [
                "Sean Connery",
                "Daniela Bianchi",
                "Pedro Armendáriz",
                "Lotte Lenya"
        ],
        "director" : "Terence Young",
        "nominaciones" : {
                "wins" : 2,
                "nominations" : 5,
                "text" : "Nominated for 1 Golden Globe. Another 2 wins & 5 nominations."
        },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
19: {
	"_id" : ObjectId("569190ce24de1e0ce2dfcd6c"),
	"titulo" : "I Love You Phillip Morris",
	"genero" : [
		"Biography",
		"Comedy",
		"Crime"
	],
	"anioPublicacion" : 2009,
	"duracion" : 98,
	"reparto" : [
		"Jim Carrey",
		"Ewan McGregor",
		"Leslie Mann",
		"Rodrigo Santoro"
	],
	"director" : "Glenn Ficarra, John Requa",
	"nominaciones" : {
		"wins" : 5,
		"nominations" : 8,
		"text" : "5 wins & 8 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
 20:  {
	"_id" : ObjectId("569190ce24de1e0ce2dfcd6d"),
	"titulo" : "2001: A Space Odyssey",
	"genero" : [
		"Mystery",
		"Sci-Fi"
	],
	"anioPublicacion" : 1968,
	"duracion" : 149,
	"reparto" : [
		"Keir Dullea",
		"Gary Lockwood",
		"William Sylvester",
		"Daniel Richter"
	],
	"director" : "Stanley Kubrick",
	"nominaciones" : {
		"wins" : 13,
		"nominations" : 7,
		"text" : "Won 1 Oscar. Another 13 wins & 7 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
21: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd6e"),
	"titulo" : "Office Space",
	"genero" : [
		"Comedy"
	],
	"anioPublicacion" : 1999,
	"duracion" : 89,
	"reparto" : [
		"Ron Livingston",
		"Jennifer Aniston",
		"David Herman",
		"Ajay Naidu"
	],
	"director" : "Mike Judge",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 2,
		"text" : "2 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
22: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd6f"),
	"titulo" : "Zathura: A Space Adventure",
	"genero" : [
		"Action",
		"Adventure",
		"Comedy"
	],
	"anioPublicacion" : 2005,
	"duracion" : 101,
	"reparto" : [
		"Jonah Bobo",
		"Josh Hutcherson",
		"Dax Shepard",
		"Kristen Stewart"
	],
	"director" : "Jon Favreau",
	"nominaciones" : {
		"wins" : 2,
		"nominations" : 3,
		"text" : "2 wins & 3 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
23: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd70"),
	"titulo" : "Space Cowboys",
	"genero" : [
		"Action",
		"Adventure",
		"Thriller"
	],
	"anioPublicacion" : 2000,
	"duracion" : 130,
	"reparto" : [
		"Clint Eastwood",
		"Tommy Lee Jones",
		"Donald Sutherland",
		"James Garner"
	],
	"director" : "Clint Eastwood",
	"nominaciones" : {
		"wins" : 4,
		"nominations" : 6,
		"text" : "Nominated for 1 Oscar. Another 4 wins & 6 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
 24 :{
	"_id" : ObjectId("569190cf24de1e0ce2dfcd71"),
	"titulo" : "Lost in Space",
	"genero" : [
		"Action",
		"Adventure",
		"Family"
	],
	"anioPublicacion" : 1998,
	"duracion" : 130,
	"reparto" : [
		"William Hurt",
		"Mimi Rogers",
		"Heather Graham",
		"Lacey Chabert"
	],
	"director" : "Stephen Hopkins",
	"nominaciones" : {
		"wins" : 1,
		"nominations" : 13,
		"text" : "1 win & 13 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
25: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd72"),
	"titulo" : "Plan 9 from Outer Space",
	"genero" : [
		"Horror",
		"Sci-Fi"
	],
	"anioPublicacion" : 1959,
	"duracion" : 79,
	"reparto" : [
		"Gregory Walcott",
		"Mona McKinnon",
		"Duke Moore",
		"Tom Keene"
	],
	"director" : "Edward D. Wood Jr.",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 0,
		"text" : ""
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
25: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd73"),
	"titulo" : "Muppets from Space",
	"genero" : [
		"Adventure",
		"Comedy",
		"Family"
	],
	"anioPublicacion" : 1999,
	"duracion" : 87,
	"reparto" : [
		"Dave Goelz",
		"Steve Whitmire",
		"Bill Barretta",
		"Jerry Nelson"
	],
	"director" : "Tim Hill",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 1,
		"text" : "1 nomination."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
26: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd74"),
	"titulo" : "Turks in Space",
	"genero" : [
		"Action",
		"Comedy",
		"Sci-Fi"
	],
	"anioPublicacion" : 2006,
	"duracion" : 110,
	"reparto" : [
		"Cüneyt Arkin",
		"Haldun Boysan",
		"Berda Ceyhan",
		"Veysel Diker"
	],
	"director" : "Kartal Tibet",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 0,
		"text" : ""
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
27: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd75"),
	"titulo" : "2001: A Space Travesty",
	"genero" : [
		"Comedy",
		"Sci-Fi"
	],
	"anioPublicacion" : 2000,
	"duracion" : 99,
	"reparto" : [
		"Leslie Nielsen",
		"Ophélie Winter",
		"Ezio Greggio",
		"Peter Egan"
	],
	"director" : "Allan A. Goldstein",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 0,
		"text" : ""
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
26: {
	"_id" : ObjectId("569190cf24de1e0ce2dfcd76"),
	"titulo" : "Space Chimps",
	"genero" : [
		"Animation",
		"Adventure",
		"Comedy"
	],
	"anioPublicacion" : 2008,
	"duracion" : 81,
	"reparto" : [
		"Andy Samberg",
		"Cheryl Hines",
		"Jeff Daniels",
		"Patrick Warburton"
	],
	"director" : "Kirk De Micco",
	"nominaciones" : {
		"wins" : 0,
		"nominations" : 1,
		"text" : "1 nomination."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
27: {
	"_id" : ObjectId("569190d024de1e0ce2dfcd77"),
	"titulo" : "The Adventures of Tintin",
	"genero" : [
		"Animation",
		"Action",
		"Adventure"
	],
	"anioPublicacion" : 2011,
	"duracion" : 107,
	"reparto" : [
		"Jamie Bell",
		"Andy Serkis",
		"Daniel Craig",
		"Nick Frost"
	],
	"director" : "Steven Spielberg",
	"nominaciones" : {
		"wins" : 22,
		"nominations" : 58,
		"text" : "Nominated for 1 Oscar. Another 22 wins & 58 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
28: {
	"_id" : ObjectId("569190d024de1e0ce2dfcd78"),
	"titulo" : "The Adventures of Baron Munchausen",
	"genero" : [
		"Adventure",
		"Comedy",
		"Fantasy"
	],
	"anioPublicacion" : 1988,
	"duracion" : 126,
	"reparto" : [
		"John Neville",
		"Eric Idle",
		"Sarah Polley",
		"Oliver Reed"
	],
	"director" : "Terry Gilliam",
	"nominaciones" : {
		"wins" : 6,
		"nominations" : 9,
		"text" : "Nominated for 4 Oscars. Another 6 wins & 9 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
29: {
	"_id" : ObjectId("569190ce24de1e0ce2dfcd68"),
	"titulo" : "Love & Other Drugs",
	"genero" : [
		"Comedy",
		"Drama",
		"Romance"
	],
	"anioPublicacion" : 2010,
	"duracion" : 112,
	"reparto" : [
		"Jake Gyllenhaal",
		"Anne Hathaway",
		"Oliver Platt",
		"Hank Azaria"
	],
	"director" : "Edward Zwick",
	"nominaciones" : {
		"wins" : 1,
		"nominations" : 5,
		"text" : "Nominated for 2 Golden Globes. Another 1 win & 5 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
},
30: {
	"_id" : ObjectId("569190ce24de1e0ce2dfcd69"),
	"titulo" : "Punch-Drunk Love",
	"genero" : [
		"Comedy",
		"Drama",
		"Romance"
	],
	"anioPublicacion" : 2002,
	"duracion" : 95,
	"reparto" : [
		"Adam Sandler",
		"Jason Andrews",
		"Don McManus",
		"Emily Watson"
	],
	"director" : "Paul Thomas Anderson",
	"nominaciones" : {
		"wins" : 14,
		"nominations" : 28,
		"text" : "Nominated for 1 Golden Globe. Another 14 wins & 28 nominations."
  },
costoUnitario: 123.78,
    copias: 2,
    createAt : new Date(),
    updateAt : new Date(),
    deleted : false
}

}
